...
function readStats() {
  try {
    ...
    if (!fs.existsSync(STATS_FILE)) {
      const initial = { views: 0, updatedAt: new Date().toISOString(), createdAt: new Date().toISOString() };
    ...
    return { views: 0, updatedAt: null, createdAt: null };
  }
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (urlObj.pathname === '/api/health') return sendJson(res, 200, { ok: true, service: 'Bel Canto Score Search', version: '1.7.3', time: new Date().toISOString() }, 'no-cache');
    if (urlObj.pathname === '/api/stats') return sendJson(res, 200, readStats(), 'no-cache');
    if (urlObj.pathname === '/api/track-view') return sendJson(res, 200, trackView(), 'no-cache');
...
server.listen(PORT, () => {
  console.log(`Bel Canto Score Search v7.2 running at http://localhost:${PORT}`);
});
