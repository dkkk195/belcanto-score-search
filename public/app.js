...
async function updateViewCounter() {
  if (!el.viewCounter) return;
  if (location.protocol === 'file:') {
    el.viewCounter.textContent = '本地预览不统计';
    return;
  }
  const sessionKey = 'belcanto_view_counted_v72';
  const endpoint = sessionStorage.getItem(sessionKey) ? '/api/stats' : '/api/track-view';
  try {
    const res = await fetch(endpoint, { cache: 'no-store' });
    if (!res.ok) throw new Error('stats unavailable');
    const data = await res.json();
    sessionStorage.setItem(sessionKey, '1');
    el.viewCounter.textContent = Number(data.views || 0).toLocaleString('zh-CN');
  } catch {
    el.viewCounter.textContent = '暂不可用';
  }
}
...
function prepareResponsiveUi() {
  ...
  const applyDeviceClass = () => {
    const screenW = Math.min(window.screen?.width || window.innerWidth, window.innerWidth || 9999);
    const isPhoneLike = screenW <= 760 || (navigator.maxTouchPoints > 1 && screenW <= 900);
    const isTouch = navigator.maxTouchPoints > 0 || matchMedia('(pointer: coarse)').matches;
    document.documentElement.classList.toggle('is-touch', isTouch);
    document.documentElement.classList.toggle('phone-ui', isPhoneLike);
  };

  applyDeviceClass();
  window.addEventListener('resize', applyDeviceClass, { passive: true });
  window.addEventListener('orientationchange', () => setTimeout(applyDeviceClass, 120), { passive: true });
}
